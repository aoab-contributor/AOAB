        ...
        public static async Task ExtractJSON()
        {
            ...
            using (var client = new HttpClient()) 
            {
                ...
                foreach (var vol in series.SelectMany(x => x.Volumes))
                {
                    ...
                    {
                        await Downloader.DoDownloadsInteractive(client, login.AccessToken, inFolder, new List<Name> { new Name { ApiSlug = vol.ApiSlug, FileName = vol.FileName } }, MangaQuality.FourK);
                    }
                }
            }
